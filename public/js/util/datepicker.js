...
function onClickPicker(datepicker) {
  if (!datepicker.inline && !datepicker.config.disableTouchKeyboard) {
    ...
    // Custom Trigger Event, delay the action to make sure form value change
    setTimeout(()=>{
      var event = new Event('change');
      datepicker.inputField.dispatchEvent(event);
    },50)
  }
}
...
class Picker {
  constructor(datepicker) {
    ...
    elementClass === 'dropdown' ? element.classList.add('dropdown', 'absolute', 'top-0', 'left-0', 'z-50', 'pt-2', 'z-[1000]') : null;
